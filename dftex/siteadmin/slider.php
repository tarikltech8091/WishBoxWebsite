...
<?php
	...
	/*Product Add*/
	if(isset($_REQUEST['slider_image_add'])){
		post_type_image_insert();
	}

	$get_post_type = get_post_type('about_slider');



	/*Slider Delete*/
	if(isset($_REQUEST['action']) && ($_REQUEST['action']=='delete') && isset($_REQUEST['sld']) && !empty($_REQUEST['sld'])){

		meta_post_delete($_REQUEST['sld']);
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<form action="" method="POST" enctype="multipart/form-data">
								...
								<input type="hidden"  class="form-control" name="post_type" value="about_slider">
							<div class="alert">
							  <span class="closebtn" onclick="this.parentElement.style.display='none';">&times;</span>
							  <table>
							  	<tr><td><strong>Acceptable Size:</strong><tr><td>
							  	<tr><td> Width :475 </td> <td> Height :303 </td></tr>
							  </table>
							</div>
							...
							<input class="btn btn-default" type="submit" value="Add" name="slider_image_add">
						</form>
					</div>
				</div>
			</div>
							...
							<div class="panel-body">
								...
								<table class="table table-striped table-bordered table-hover">
									...
									<tbody>
	                                    ...
	                                    $all_slider_image_list = get_post_image_pagination('about_slider',$start_from,$num_rec_per_page);
										...
										if(isset($all_slider_image) && count($all_slider_image) > 0){
											...
											foreach ($all_slider_image as $key => $list) {
												...
												echo '<a onclick="SliderconfirmDelete('.$list['meta_id'].')" class="btn btn-xs btn-bricky tooltips" data-placement="top" data-original-title="Remove"><i class="fa fa-times fa fa-white"></i></a>';
												echo '</td>';
												echo '</tr>';
...
<script>
    function SliderconfirmDelete(meta_id) {
     
        var r = confirm("Are you want to Delete??");
        if (r == true) {
           window.location ='slider.php?action=delete&sld='+meta_id;
        } else {
            return false;
        }  
    }
