...
<?php
	...
	/*Product Add*/
	if(isset($_REQUEST['client_image_add'])){
		post_type_image_insert();
	}

	$get_post_type = get_post_type('client');


	/*Client Delete*/
	if(isset($_REQUEST['action']) && ($_REQUEST['action']=='delete') && isset($_REQUEST['cnt']) && !empty($_REQUEST['cnt'])){

		meta_post_delete($_REQUEST['cnt']);
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<form action="" method="POST" enctype="multipart/form-data">
								...
								<input type="hidden"  class="form-control" name="post_type" value="client">
							<div class="alert">
							  <span class="closebtn" onclick="this.parentElement.style.display='none';">&times;</span>
							  <table>
							  <tr><td><strong>Acceptable Size:</strong><tr><td>
							  	<tr><td> Width :198 </td> <td> Height :198 </td></tr>
							  </table>
							</div>
							...
							<input class="btn btn-default" type="submit" value="Add" name="client_image_add">
						</form>
					</div>
				</div>
			</div>
							...
							<div class="panel-body">
								...
								<table class="table table-striped table-bordered table-hover">
									...
									<tbody>
	                                    ...
	                                    $all_client_image_list = get_post_image_pagination('client',$start_from,$num_rec_per_page);

	                                    $all_client_image= $all_client_image_list['all_post_type'];
	                                    $total_records = $all_client_image_list['total_records'];

										if(isset($all_client_image) && count($all_client_image) > 0){

											foreach ($all_client_image as $key => $list) {
												echo "<tr>";
												echo '<td>'.($key+1).'</td>';
												...
												echo '<a onclick="ClientconfirmDelete('.$list['meta_id'].')" class="btn btn-xs btn-bricky tooltips" data-placement="top" data-original-title="Remove"><i class="fa fa-times fa fa-white"></i></a>';
												echo '</td>';
												echo '</tr>';
...
<script>
    function ClientconfirmDelete(meta_id) {
     
        var r = confirm("Are you want to delete client image??");
        if (r == true) {
           window.location ='client.php?action=delete&cnt='+meta_id;
        } else {
            return false;
        }  
    }
